...
function login(username, password) {
  if (username === credentials.username && password === credentials.password) {
    ...
    // Save to a more persistent location if needed, but localStorage is usually enough
...
// Replace DOMContentLoaded listener with one that checks status
  ...
  try {
    ...
    snapshot.docs.sort((a, b) => (b.data().timestamp?.seconds || 0) - (a.data().timestamp?.seconds || 0)).forEach(doc => {
      ...
      row.innerHTML = `
        ...
        <td><button class="btn btn-sm btn-outline" onclick="viewUserDocs('${booking.userId || ''}')">View IDs</button></td>
        <td>
